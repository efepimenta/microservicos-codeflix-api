...
class GenreHasCategoriesUnitTest extends TestCase
{
    public function testIfClassInstanceOfRule()
    {
        $rule = new GenresHasCategoriesRules([1, 2, 3, 4]);
        $this->assertInstanceOf(GenresHasCategoriesRules::class, $rule);
    }
    ...
    public function testPassesReturnsFalseWhenCategoriesOrGenreIsEmpty()
    {
    ...
    public function testPassesReturnFalseWhenRowsIsEmpty()
    {
        $rule = $this->createRuleMock([1]);
        $rule->shouldReceive('getRows')
            ->withAnyArgs()
    ...
    public function testPassesReturnsFalseWhenHasGenresWithoutCategories()
    {
        $rule = $this->createRuleMock([1]);
        $rule->shouldReceive('getRows')
            ->withAnyArgs()
    ...
    private function createRuleMock(array $values): MockInterface
    {
        return \Mockery::mock(GenresHasCategoriesRules::class, [$values])
            ->makePartial()
